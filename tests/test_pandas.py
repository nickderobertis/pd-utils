...
class TestCumulate(DataFrameTest):
    ...
    def test_method_between_1_3(self):
        cum_df = pd_utils.cumulate(self.df, 'RET', 'between', periodvar='Date',
    ...
    def test_shifted_index(self):
        ...
        cum_df = pd_utils.cumulate(df, 'RET', 'between', periodvar='Date',
                                      byvars=['PERMNO','byvar'], time=[-2,0])

        assert_frame_equal(self.expect_between_1_3, cum_df, check_dtype=False)

    def test_method_first(self):
        cum_df = pd_utils.cumulate(self.df, 'RET', 'first', periodvar='Date',
                                      byvars=['PERMNO','byvar'])
    ...
    def test_grossify(self):
        ...
        cum_df = pd_utils.cumulate(df, 'RET', 'first', periodvar='Date',
...
class TestGroupbyMerge(DataFrameTest):
    ...
    def test_nan_byvar_transform(self):
        ...
        out = pd_utils.groupby_merge(self.df_nan_byvar, 'byvar', 'transform', (lambda x: x))
    ...
    def test_nan_byvar_and_nan_val_transform_numeric(self):
        ...
        out = pd_utils.groupby_merge(non_standard_index, 'byvar', 'transform', (lambda x: x + 1))
...
class TestLongToWide:
    ...
    ltw_no_dup_colindex    = pd_utils.long_to_wide(input_data.df,
                                                     ['PERMNO', 'byvar'], 'RET', colindex='Date')
    ltw_dup_colindex       = pd_utils.long_to_wide(input_data.df_duplicate_row,
                                                     ['PERMNO', 'byvar'], 'RET', colindex='Date')
    ltw_no_dup_no_colindex = pd_utils.long_to_wide(input_data.df,
                                                     ['PERMNO', 'byvar'], 'RET')
    ltw_dup_no_colindex    = pd_utils.long_to_wide(input_data.df_duplicate_row,
                                                     ['PERMNO', 'byvar'], 'RET')
...
class TestPortfolioAverages:
    ...
    avgs, ports = pd_utils.portfolio_averages(input_data.df_weight, 'RET', 'RET', ngroups=2,
                                                  byvars='byvar')

    w_avgs, w_ports = pd_utils.portfolio_averages(input_data.df_weight, 'RET', 'RET', ngroups=2,
...
class TestWinsorize(DataFrameTest):
    ...
    def test_winsor_40_subset_byvars(self):
        ...
        wins = pd_utils.winsorize(self.df, .4, subset='RET', byvars=['PERMNO','byvar'])
...
class TestConvertSASDateToPandasDate:
    ...
    def test_convert(self):
        ...
        converted =  pd.DataFrame(pd_utils.convert_sas_date_to_pandas_date(self.df_sasdate['datadate']))
    ...
    def test_convert_nan(self):
        ...
        converted =  pd.DataFrame(pd_utils.convert_sas_date_to_pandas_date(self.df_sasdate_nan['datadate']))
...
class TestMapWindows(DataFrameTest):
    ...
    def test_method_first(self):
        ...
        result = pd_utils._map_windows(self.df_period, self.times[0], method='first',
    ...
    def test_method_between(self, time, expect_df):
        ...
        result = pd_utils._map_windows(self.df_period, time, method='between',
                                          periodvar='Date', byvars=['PERMNO'])
...
class TestLeftMergeLatest(DataFrameTest):
    ...
    def test_left_merge_latest(self):
        ...
        lm = pd_utils.left_merge_latest(self.df_gvkey_str, self.df_gvkey_str2, on='GVKEY')
        lm_low_mem = pd_utils.left_merge_latest(self.df_gvkey_str, self.df_gvkey_str2, on='GVKEY', low_memory=True)
        lm_sql = pd_utils.left_merge_latest(self.df_gvkey_str, self.df_gvkey_str2,
...
class TestVarChangeByGroups(DataFrameTest):
    ...
    def test_multi_byvar_multi_var(self):
        ...
        vc = pd_utils.var_change_by_groups(self.df_weight, ['RET','weight'], ['PERMNO','byvar'])
...
class TestFillExcludedRows(DataFrameTest):
    ...
    def test_no_fillvars_series_byvars(self):
        result = pd_utils.fill_excluded_rows(self.df_gvkey_str, [self.df_gvkey_str['GVKEY'],'Date'])
        assert_frame_equal(self.expect_df_nofill, result)
...
class TestFillnaByGroups(DataFrameTest):
    ...
    def test_fillna_by_group_keep_one(self):
        ...
        result = pd_utils.fillna_by_groups_and_keep_one_per_group(self.df_fill_data, 'group')
